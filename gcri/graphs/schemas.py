...
class RawHypothesis(BaseModel):
    """Hypothesis generated by BranchesGenerator before verification."""
    index: int = Field(..., description='Original branch index (0-based)')
    ...
    container_id: str = Field(..., description='Docker container or local sandbox ID')


class AggregatedBranch(BaseModel):
    """Branch output from Aggregating Agent after merging/filtering."""
    index: int = Field(..., description='New branch index after aggregation (0-based)')
...
class AggregationResult(BaseModel):
    """Output schema for the Aggregating Agent."""
    branches: List[AggregatedBranch] = Field(..., description='List of aggregated branches')
    discarded_indices: List[int] = Field(
