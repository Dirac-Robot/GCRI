...
if INSPECT_AVAILABLE:
    ...
    async def _solve_with_inspect_sandbox(
        ...
        # Patch both module-level reference AND the utils.py/cli.py local references
        import gcri.tools.utils as utils_module
        import gcri.tools.cli as cli_module
        original_utils_get_docker_sandbox = utils_module.get_docker_sandbox
        original_cli_get_docker_sandbox = cli_module.get_docker_sandbox
        ...
        cli_module.get_docker_sandbox = patched_get_docker_sandbox
        ...
        # Truncate input if too long (GAIA context overflow fix)
        max_input_chars = 300000  # Approx 75k tokens
        if len(task_input) > max_input_chars:
            logger.warning(f"Task input too long ({len(task_input)} chars). Truncating to {max_input_chars} chars.")
            task_input = task_input[:max_input_chars] + "\n...[TRUNCATED DUE TO LENGTH]..."
        ...
        try:
            ...
            cli_module.get_docker_sandbox = original_cli_get_docker_sandbox
            docker_sandbox_module._docker_sandbox_instance = None
        return state
...
class GCRIBenchmark(ABC):
    ...
    def make_task(self) -> 'Task':
        ...
        # If scorer is None and make_official_task exists, use official task
        if scorer is None and hasattr(self, 'make_official_task'):
            return self.make_official_task()
